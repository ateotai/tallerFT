...
(async () => {
  ...
  // En producción forzamos 0.0.0.0 para evitar problemas de accesibilidad en Railway
  const rawDevHost = (process.env.BIND_HOST || process.env.HOST || '127.0.0.1');
  const bindHost = process.env.NODE_ENV === 'production'
    ? '0.0.0.0'
    : rawDevHost.trim();
  server.listen({
    port,
