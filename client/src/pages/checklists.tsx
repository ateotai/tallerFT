...
import { Trash2, Printer, Eye, Pencil } from "lucide-react";
...
export default function ChecklistsPage() {
  ...
  const { data: checklists = [] } = useQuery<Checklist[]>({
    queryKey: ["/api/checklists", { status: statusFilter, economicNumber: economicSearch, start: startDate, end: endDate, vehicleId: vehicleFilter }],
    queryFn: async () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Revisión eliminada", description: "El registro fue eliminado exitosamente" });
    },
    onError: (error: Error) => {
      toast({ title: "Error", description: error.message || "No se pudo eliminar la revisión", variant: "destructive" });
    },
  });
  ...
  function handlePrint(checklist: Checklist) {
    ...
    const statusRaw = String((checklist as any).status || "");
    const statusText = statusRaw === "operando_con_falla" ? "Operando con falla" : "Operando";
    const statusClass = statusRaw === "operando_con_falla" ? "red" : "green";
    ...
    const html = `
      <html>
        <head>
          <title>Revisión ${checklist.folio || checklist.id}</title>
          <meta charset="utf-8" />
          <style>
            ...
            .badge { display: inline-block; font-size: 12px; padding: 2px 8px; border-radius: 9999px; color: #fff; }
            .badge.green { background: #16a34a; }
            .badge.red { background: #dc2626; }
          </style>
        </head>
        <body>
          <h1>Revisión ${checklist.folio || checklist.id}</h1>
          ...
          <div class="section grid">
            ...
            <div><strong>Estado:</strong> <span class="badge ${statusClass}">${statusText}</span></div>
          </div>
          ${vehicleDetails}
          ${sections}
          ${general}
  ...
  function canEdit(c: Checklist): boolean {
    const roleText = (user?.role || '').toLowerCase();
    const isAdmin = roleText === 'admin' || roleText === 'administrador';
    const creatorId = (c as any).createdByUserId as number | undefined;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold">Tareas de Revision</h2>
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              ...
              <SelectContent>
                ...
                <SelectItem value="operando_con_falla">Operando con falla</SelectItem>
              </SelectContent>
            </Select>
            <Input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-40" />
            <Input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="w-40" />
            <Button variant="ghost" onClick={() => { setStatusFilter("all"); setEconomicSearch(""); setStartDate(""); setEndDate(""); setVehicleFilter(null); }}>Limpiar</Button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {checklists.map((c) => (
                <div key={c.id} className="border rounded-md p-4">
                  ...
                  <div className="mt-1">
                    <span className={`text-xs px-2 py-0.5 rounded text-white ${String((c as any).status||'') === 'operando_con_falla' ? 'bg-red-600' : 'bg-green-600'}`}>
                      {String((c as any).status||'') === 'operando_con_falla' ? 'Operando con falla' : 'Operando'}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    ...
                    {canEdit(c) && (
                      <Button
                        ...
                        data-testid={`button-edit-checklist-${c.id}`}
                      >
                  ...
                  {checklists.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                        {economicSearch || startDate || endDate || (statusFilter && statusFilter !== "all") || vehicleFilter ? "No se encontraron revisiones" : "No hay revisiones registradas"}
                      </TableCell>
                    </TableRow>
                  ) : (
                    checklists.map((c) => (
                      <TableRow key={c.id}>
                        ...
                        <TableCell>{c.evidenceUrl ? (<a href={c.evidenceUrl as any} target="_blank" rel="noreferrer" className="text-blue-600 underline">Abrir</a>) : "-"}</TableCell>
                        ...
                        <TableCell className="text-right">
                          ...
                          <Button variant="ghost" size="icon" title="Ver" onClick={() => setViewingChecklist(c)} data-testid={`button-view-checklist-${c.id}`}>
                            ...
                            {canEdit(c) && (
                              <Button variant="outline" size="icon" title="Editar" onClick={() => setEditingChecklist(c)} data-testid={`button-edit-checklist-${c.id}`}>
                                <Pencil className="h-4 w-4" />
                              </Button>
                            )}
      ...
      <Dialog open={!!viewingChecklist} onOpenChange={(o) => !o && setViewingChecklist(null)}>
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          {viewingChecklist && (
            <>
              <DialogHeader>
                <DialogTitle>Revisión {viewingChecklist.folio || viewingChecklist.id}</DialogTitle>
                <DialogDescription>{new Date(viewingChecklist.inspectedAt as any).toLocaleString()}</DialogDescription>
                <div className="mt-2">
                  <Button variant="outline" onClick={() => handlePrint(viewingChecklist)}>
              ...
              <div className="space-y-2">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded text-white ${String((viewingChecklist as any).status||'') === 'operando_con_falla' ? 'bg-red-600' : 'bg-green-600'}`}>
                      {String((viewingChecklist as any).status||'') === 'operando_con_falla' ? 'Operando con falla' : 'Operando'}
                    </span>
                  </div>
                </div>
                <div className="mt-4">
                ...
                <div className="mt-4 space-y-2">
                  ...
                  {viewingChecklist.evidenceUrl && (
                    <div className="space-y-1">
                      <div className="font-medium">Evidencia</div>
                      <div className="text-sm"><a href={viewingChecklist.evidenceUrl as any} target="_blank" rel="noreferrer" className="text-blue-600 underline">Abrir archivo</a></div>
                    </div>
                  )}
      ...
      {editingChecklist && (
        <AddChecklistDialog
          ...
          onOpenChange={(o) => { if (!o) setEditingChecklist(null); }}
